...
class NotasController {
  public async index(req: Request, res: Response) {
    await pool.then((r) => {
      let result = r.query("select alumno.nombre, alumno.apellidos, notas.nota, asignatura.nombre_curso from notas inner join asignatura  on notas.asignatura_id = asignatura.id inner join alumno on notas.id_alumno = alumno.id", function (
        error: Error,
        rows: any
      ) {
         res.send(rows);
